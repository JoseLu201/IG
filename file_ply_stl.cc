  ...
  if(Token=="ply"){
    ...
    if(Token=="format"){
      ...
      if(Token=="ascii"){
        ...
        if(Token=="vertex"){
          get_token(Line_stream,Token);
          Coordinates.resize(stoi(Token)*3); //Error en armadillo token es "" cuando tiene que ser un string numero
          do{
            get_new_line(File,Line_stream);
            get_token(Line_stream,Token);
          } while (Token!="element");
...
{   //cerr << "Cerrando el archivo" << endl;
  File.close();
//cerr << "El archivo ha sido cerrado" << endl;
}
